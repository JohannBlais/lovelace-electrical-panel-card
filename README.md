...
- **Source summary** — mark any group `summary: true` and it gets a row above the diagram with its accent and its live reading. On a board reachable by several paths (transfer switch, inverter bypass) the one carrying the house is the one not reading zero. The same key on `sensors.total` / `sensors.grid` / a phase *moves* that reading off the board into the foot of the same table, so a figure never shows up twice
...
```yaml
resources:
  - url: /local/electrical-panel-card/electrical-panel-card.js?v=0.20.0
    type: module
```
...
That rewrites `package.json` and the lockfile without committing or tagging. Add a [`CHANGELOG.md`](CHANGELOG.md) entry for the new version, land both as a release PR, then tag the merge commit — one command at a time:
...
Deliberately not chained with `&&`: this repo is released from Windows PowerShell 5.1, where `&&` is not a valid statement separator and the whole line fails to parse. `$( … )` is fine there — it is only the chaining that breaks. Keep them separate if you edit this section.
